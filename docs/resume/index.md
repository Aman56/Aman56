---
layout: single
author_profile: true
title: "Resume"
permalink: /resume/
---

<div class="page-nav">
  <a href="/">Home</a> · <a href="/publications/">Publications</a> · <a href="/projects/">Projects</a> · <a href="/resume/">Resume</a> · <a href="/news/">News</a> · <a href="/network/">Network</a>
</div>

<div style="text-align: center; margin-bottom: 1.5em;">
  <a href="/assets/Aman%20-%20Recent.pdf" class="btn btn--primary">Download Full Resume (PDF)</a>
</div>

## Key Credentials

- **ICLR 2024:** Grokking and circuit competition paper published
- **Production Impact:** +5% lift on personalization baseline; 3T-row AWS inference deployment
- **2025 Preprint:** User behavior modeling framework with adaptive surveys
- **NYU Master's:** Computer Science (Thesis: Mechanistic interpretability and generalization)

---

## Experience

<div class="timeline">
  <div class="timeline-item">
    <div class="timeline-header">
      <span class="timeline-role">AI/ML Scientist</span>
      <span class="timeline-company">Resonate Networks</span>
      <span class="timeline-date">2023–Present</span>
    </div>
    <ul class="timeline-bullets">
      <li>Designed and deployed end-to-end personalization pipeline: adaptive surveys + synthetic respondent modeling + distributed inference (PyTorch, AWS)</li>
      <li>Achieved +5% lift over XGBoost baseline on audience prediction; deployed to 3T-row inference (p99 <200ms latency)</li>
      <li>Built model monitoring and retraining infrastructure to sustain performance under data drift</li>
      <li>Led cross-functional collaboration with product and data teams; shipped multiple production models</li>
      <li>Published methodology in 2025 preprint; presented work internally and externally</li>
    </ul>
  </div>

  <div class="timeline-item">
    <div class="timeline-header">
      <span class="timeline-role">ML Research Lead</span>
      <span class="timeline-company">NYU Computer Science (Prof. Gus Xia)</span>
      <span class="timeline-date">2021–2023</span>
    </div>
    <ul class="timeline-bullets">
      <li>Investigated mechanistic interpretability through circuit analysis and sparse-dense network competition</li>
      <li>Published grokking paper at ICLR 2024; developed interpretability framework used in follow-up research</li>
      <li>Researched music emotion transfer and representation learning (EmoMusic project)</li>
      <li>Mentored 2 junior researchers on interpretability methodology and experimental design</li>
      <li>Collaborated with Anthropic and Distill community on interpretability standards</li>
    </ul>
  </div>

  <div class="timeline-item">
    <div class="timeline-header">
      <span class="timeline-role">Autonomous Driving Intern</span>
      <span class="timeline-company">Capgemini Applied Innovation Exchange</span>
      <span class="timeline-date">Summer 2022</span>
    </div>
    <ul class="timeline-bullets">
      <li>Developed orthogonal coordinate transformation framework for multi-sensor fusion (LiDAR, camera, GPS)</li>
      <li>Achieved 10% accuracy improvement over baseline; deployed on embedded systems</li>
      <li>Validated on real-world autonomous vehicle dataset; integrated with production platform</li>
    </ul>
  </div>
</div>

---

## Education

<div class="timeline">
  <div class="timeline-item">
    <div class="timeline-header">
      <span class="timeline-role">Master of Science, Computer Science</span>
      <span class="timeline-company">New York University</span>
      <span class="timeline-date">2021–2023</span>
    </div>
    <ul class="timeline-bullets">
      <li><strong>Thesis:</strong> Mechanistic Interpretability and Generalization in Neural Networks</li>
      <li><strong>Advisor:</strong> Prof. Gus Xia</li>
      <li><strong>Focus:</strong> Machine learning, mechanistic interpretability, deep learning</li>
    </ul>
  </div>

  <div class="timeline-item">
    <div class="timeline-header">
      <span class="timeline-role">Bachelor of Science, Computer Science</span>
      <span class="timeline-company">Undergraduate</span>
      <span class="timeline-date">2017–2021</span>
    </div>
    <ul class="timeline-bullets">
      <li>Cumulative GPA: 3.8/4.0</li>
      <li>Focus: Machine learning, software engineering, mathematics</li>
    </ul>
  </div>
</div>

---

## Skills

<div class="method-groups">
  <div class="method-group">
    <span class="method-label">Modeling & ML</span>
    <span class="method-items">Deep learning (PyTorch, JAX), mechanistic interpretability, circuit analysis, representation learning, reinforcement learning, personalization systems, survey design</span>
  </div>
  <div class="method-group">
    <span class="method-label">Data & Infrastructure</span>
    <span class="method-items">Distributed training (PyTorch DDP), AWS (EC2, S3, RDS), PostgreSQL, Redis, data pipelines, ETL, monitoring & observability, model serving (FastAPI)</span>
  </div>
  <div class="method-group">
    <span class="method-label">Languages & Tools</span>
    <span class="method-items">Python, C++, SQL, Bash, Docker, Git, Jupyter, Matplotlib, NumPy, Pandas, LibROSA</span>
  </div>
</div>

---

## Publications

<ul class="pub-item">
  <span class="pub-title">Grokking as Competitive Pressure Between Sparsely and Densely Connected Subnetworks</span>
  <span class="pub-venue">ICLR 2024</span>
  <a href="https://arxiv.org/abs/2311.12982" class="pub-link">arXiv</a> · <a href="https://scholar.google.com/citations?user=dtBDI_oAAAAJ&hl=en&oi=ao" class="pub-link">Scholar</a>
</ul>

<ul class="pub-item">
  <span class="pub-title">Towards Better Personalization: Modeling User Preferences via Synthetic Respondents</span>
  <span class="pub-venue">2025 Preprint</span>
  <a href="https://scholar.google.com/citations?user=dtBDI_oAAAAJ&hl=en&oi=ao" class="pub-link">Scholar</a> · <a href="https://github.com/Aman56" class="pub-link">Code</a>
</ul>

---

## Notable Projects

- **Audience Personalization (Production):** +5% lift, 3T-row inference deployment, measurable business impact
- **Mechanistic Interpretability Research:** Published at ICLR 2024, community impact
- **EmoMusic:** Emotion transfer and representation learning research
- **Autonomous Driving (Capgemini):** Multi-sensor fusion, coordinate transformation framework

---

## About This Resume

This is my resume focused on recent work and key projects. A more comprehensive CV can be prepared if needed. See [Publications](/publications/) and [Projects](/projects/) for more details.

---

<div class="page-nav">
  <a href="/">Home</a> · <a href="/publications/">Publications</a> · <a href="/projects/">Projects</a> · <a href="/resume/">Resume</a> · <a href="/news/">News</a> · <a href="/network/">Network</a>
</div>
